...
public:
  ...
  : nr_dofs_(number_of_dofs),
    current_control_mode_(control_mode_type::POSITION),
  ...
  control_mode_type get_control_mode()
  {
  ...
  {
    // 本来はハードからセンサ値を読み取るが、ここではシミュレーション
  ...
  {
    // 本来はハードからGPIOの状態を読み取るが、ここではシミュレーション（ランダムな2値を生成）
  ...
  {
    // 本来はハードからセンサ値を読み取るが、ここではシミュレーション（ランダムな値を生成）
